...
int contarConsoantesRecursivamente(char **texto, int indice) {
    char letraAtual = *(*texto + indice);
    ...
    if (isalpha(letraAtual)) {
        ...
        if (letraAtual != 'a' && letraAtual != 'e' && letraAtual != 'i' && letraAtual != 'o' && letraAtual != 'u') {
            return 1 + contarConsoantesRecursivamente(texto, indice + 1);
        }
    }
    
    return contarConsoantesRecursivamente(texto, indice + 1);
}
