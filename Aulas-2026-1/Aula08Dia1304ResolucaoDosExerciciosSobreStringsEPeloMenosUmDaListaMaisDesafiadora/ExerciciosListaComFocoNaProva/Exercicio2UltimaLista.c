...
int filtrarPorFaixa(int *ponteiroComOsValoresOriginais, int tamanho, int limiteInferior, int limiteSuperior){
    ...
    // o ponteiro ponteiroComValoresDentroDaFaixa neste momento está apontando para a última posição onde foi colocado um valor que está dentro da faixa, ou seja,
    // a próxima posição que ele aponta é a primeira posição onde deve ser colocado o valor 0, caso haja sobras de posições no vetor original
    while(ponteiroComValoresDentroDaFaixa < ponteiroComOsValoresOriginais + tamanho){ // enquanto o ponteiro com os valores dentro da faixa não ter o mesmo tamanho do vetor original
        *ponteiroComValoresDentroDaFaixa = 0; // preenche as posições restantes com 0
        ponteiroComValoresDentroDaFaixa++; // faz o ponteiro andar sempre para a próxima posição para que seja possível preencher com 0
    }
